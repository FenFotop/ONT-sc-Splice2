...
option_parser=OptionParser(
  usage="%prog [options] <name>_counts_sc_txt.gz path/to/output_folder output_prefix path/to/annotation_code/prefix_ [path/to/te.bed.gz]"
)
...
# --- TE Junction Annotation ---
# For each junction, flag whether either splice site (5' or 3') falls within a TE region.
# Outputs (added to all.introns):
#   te_names_5p     : "|"-separated TE names overlapping the 5' splice site (or ".")
#   te_names_3p     : "|"-separated TE names overlapping the 3' splice site (or ".")
#   has_TE_5p       : logical - 5' splice site inside a TE
#   has_TE_3p       : logical - 3' splice site inside a TE
#   has_TE_junction : logical - either splice site inside a TE
#   TE_junction_type: "TE_fiveprime" | "TE_threeprime" | "TE_both" | "none"
...
if (has_te_bed) {
  ...
  te_db <- fread(te_bed_file, data.table = TRUE)
  # expect at minimum: chr, start, end, te_name (BED cols 1-4)
  colnames(te_db)[1:4] <- c("te_chr", "te_start", "te_end", "te_name")
  te_db[, te_chr := add_chr(te_chr)]

  all.introns[, row_id := .I]

  # 5' splice site (junction start coordinate) overlaps a TE
  te_5p <- te_db[all.introns,
    on = .(te_chr == chr, te_start <= start, te_end >= start),
    .(row_id = i.row_id, te_name),
    allow.cartesian = TRUE, nomatch = NA]
  te_5p_agg <- te_5p[!is.na(te_name),
    .(te_names_5p = paste(unique(te_name), collapse = "|")), by = row_id]

  # 3' splice site (junction end coordinate) overlaps a TE
  te_3p <- te_db[all.introns,
    on = .(te_chr == chr, te_start <= end, te_end >= end),
    .(row_id = i.row_id, te_name),
    allow.cartesian = TRUE, nomatch = NA]
  te_3p_agg <- te_3p[!is.na(te_name),
    .(te_names_3p = paste(unique(te_name), collapse = "|")), by = row_id]

  all.introns[, `:=`(te_names_5p = ".", te_names_3p = ".")]
  all.introns[te_5p_agg, on = "row_id", te_names_5p := i.te_names_5p]
  all.introns[te_3p_agg, on = "row_id", te_names_3p := i.te_names_3p]
  all.introns[, `:=`(
    has_TE_5p       = te_names_5p != ".",
    has_TE_3p       = te_names_3p != ".",
    has_TE_junction = te_names_5p != "." | te_names_3p != "."
  )]
  all.introns[, TE_junction_type := fcase(
    has_TE_5p & has_TE_3p,   "TE_both",
    has_TE_5p & !has_TE_3p,  "TE_fiveprime",
    !has_TE_5p & has_TE_3p,  "TE_threeprime",
    default = "none"
  )]
  all.introns[, row_id := NULL]

  print(paste0("Junctions with TE at 5' splice site : ", sum(all.introns$has_TE_5p)))
  print(paste0("Junctions with TE at 3' splice site : ", sum(all.introns$has_TE_3p)))
  print(paste0("Total junctions with TE involvement : ", sum(all.introns$has_TE_junction)))
} else {
  all.introns[, `:=`(
    te_names_5p     = ".",
    te_names_3p     = ".",
    has_TE_5p       = FALSE,
    has_TE_3p       = FALSE,
    has_TE_junction = FALSE,
    TE_junction_type = "none"
  )]
  if (te_bed_file != "NULL") warning("TE BED file not found: ", te_bed_file)
}
# --- End TE Junction Annotation ---
